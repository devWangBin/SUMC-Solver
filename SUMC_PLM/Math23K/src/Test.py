...
def test_for_mwp_slover2(args, false_file_name: str = None):
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    ...
    total_label_wrong_count = [0] * args.num_labels
    ...
    if args.test_data_path and os.path.exists(args.test_data_path):
        ...
        for idd, raw_mwp in enumerate(pbar):
            ...
            if result_sens is not None:
                ...
                if args.use_new_token_type_id:
                    new_token_type_ids = []
                    for num_ps in num_pos:
                        tem = [0] * args.test_dev_max_len
                        for pp in num_ps:
                            tem[pp + 1] = 1
                        new_token_type_ids.append(tem)
                    new_token_type_ids = torch.tensor(new_token_type_ids).long()
