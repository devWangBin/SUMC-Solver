    ...
    def __init__(self, cached_path: str, file_path: str, tokenizer: BertTokenizer, label2id_data_path: str,
        ...
        if os.path.exists(cached_file) and use_cache:
            ...
            for idd, raw_mwp in enumerate(pbar):
                ...
                result_sens, result_labels, num_postions = process_mwp_V1(raw_mwp, self.label2id_or_value, self.max_len)
...
def process_mwp_V1(raw_mwp: dict, label2id_or_value: dict, max_len: int):
    sentence = raw_mwp['T_question_2'].split(' ')
    add_sentence = raw_mwp['T_question_2']
    ...
    for kk, vv in raw_mwp['T_number_map'].items():
        ...
        if kk not in num_codes:
            ...
            for position in postions_in_q:
                ...
                result_sens.append(add_sentence)
    ...
    def __init__(self, cached_path: str, file_path: str, tokenizer: BertTokenizer, label2id_data_path: str,
        ...
        if os.path.exists(cached_file) and use_cache:
            ...
            for idd, raw_mwp in enumerate(pbar):
                ...
                result_sens_ids, result_labels, num_postions, total_attentions = process_mwp_encode_mawps(raw_mwp,
                                                                                                          label2id_or_value,
                                                                                                          max_len)
                if result_sens_ids is not None:
                    sens_ids.extend(result_sens_ids)
                    labels.extend(result_labels)
                    num_pos.extend(num_postions)
                    Attentionss.extend(total_attentions)
                else:
                    continue

                if idd == 0:
                    print(result_sens_ids[0])
                    print(result_labels)
                    print(num_postions[0])
                    print(total_attentions[0])

            assert len(sens_ids) == len(labels) == len(num_pos) == len(Attentionss)

            res = {}
            self.input_ids = torch.tensor(sens_ids).long()
            self.attention_mask = torch.tensor(Attentionss).long()
            res['input_ids'] = self.input_ids
            res['attention_mask'] = self.attention_mask
...
def process_mwp_encode_mawps(raw_mwp: dict, label2id_or_value: dict, max_len: int, tokenizer: BertTokenizer):
    sentence = raw_mwp['T_question_2'].lower().split(' ')
    if len(sentence) > (max_len - 2):
        return None, None
    sen_ids = tokenizer.convert_tokens_to_ids(['CLS'] + sentence + ['SEP'])
    ...
    for kk, vv in raw_mwp['T_number_map'].items():
        ...
        if kk not in num_codes:
            ...
            for position in postions_in_q:
                ...
                result_sens_ids.append(sen_ids)
                result_labels.append(label_vector)
    ...
    total_all_num_postions = []
    for i in range(len(result_sens_ids)):
        total_all_num_postions.append(all_num_postions)

    attention_mask = [1] * len(sen_ids) + [0] * (max_len - len(sen_ids))

    total_attention_mask = []
    for i in range(len(result_sens_ids)):
        total_attention_mask.append(attention_mask)

    return result_sens_ids, result_labels, total_all_num_postions, total_attention_mask
