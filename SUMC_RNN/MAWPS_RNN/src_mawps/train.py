...
def train(args):
    ...
    train_data_set = MwpDataSet(cached_path=args.cached_data_path, file_path=args.train_data_path, tokenizer=tokenizer,
                                label2id_data_path=args.label2id_path, max_len=args.train_max_len, has_label=True,
                                use_new_replace=args.use_new_replace)
    ...
    dev_data_set = MwpDataSet(cached_path=args.cached_data_path, file_path=args.dev_data_path, tokenizer=tokenizer,
                              label2id_data_path=args.label2id_path, max_len=args.test_dev_max_len, has_label=True,
                              use_new_replace=args.use_new_replace)
...
def train_para(args, total_best_acc: float):
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu_device
    args.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    logger.info("define model...")
    model = RNNModel(vocab_size=args.vocab_size, embedding_size=args.embedding_size, hidden_size=args.hidden_size,
                     num_layer=args.rnn_layer, fc_size=args.fc_size, rnn_cell=args.rnn_cell,
                     num_labels=args.num_labels, fc_path=args.fc_path, multi_fc=args.multi_fc, drop_p=args.dropout)
